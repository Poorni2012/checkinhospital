...
export class ChangePasswordComponent implements OnInit {
  ...
  changePassword(changePasswordForm: NgForm) {
    ...
    if (changePasswordForm.valid) {
      ...
      let changeObjj = {
        password: this.changeObj.current_password,
        newPassword: this.changeObj.password,
        confirmPassword: this.changeObj.confirm_password,
        token: this.token
      }
      console.log("this", changeObjj)
      this.apiService.postRequest('v1/auth/changepassword', changeObjj).subscribe((passwordResponse: any) => {
